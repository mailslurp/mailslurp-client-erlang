...
-export([create_new_email_address/1, create_new_email_address/2,
         ...
         send_email_query/2, send_email_query/3,
...
create_new_email_address(Ctx, Optional) ->
    ...
    QS = lists:flatten([])++maislurp-client-erlang_utils:optional_params(['allowTeamAccess', 'useDomainPool', 'expiresAt', 'expiresIn', 'emailAddress', 'inboxType', 'description', 'name', 'tags', 'favourite', 'virtualInbox', 'useShortAddress', 'domainName', 'domainId', 'prefix'], _OptionalParams),
    Headers = [],
    Body1 = [],
...
%% @doc Send an email using query parameters
...
-spec send_email_query(ctx:ctx(), binary()) -> {ok, [], maislurp-client-erlang_utils:response_info()} | {ok, hackney:client_ref()} | {error, term(), maislurp-client-erlang_utils:response_info()}.
send_email_query(Ctx, To) ->
    send_email_query(Ctx, To, #{}).

-spec send_email_query(ctx:ctx(), binary(), maps:map()) -> {ok, [], maislurp-client-erlang_utils:response_info()} | {ok, hackney:client_ref()} | {error, term(), maislurp-client-erlang_utils:response_info()}.
send_email_query(Ctx, To, Optional) ->
    _OptionalParams = maps:get(params, Optional, #{}),
    Cfg = maps:get(cfg, Optional, application:get_env(kuberl, config, #{})),

    Method = post,
    Path = ["/sendEmailQuery"],
    QS = lists:flatten([{<<"to">>, To}])++maislurp-client-erlang_utils:optional_params(['senderId', 'body', 'subject'], _OptionalParams),
    Headers = [],
    Body1 = [],
    ContentTypeHeader = maislurp-client-erlang_utils:select_header_content_type([]),
    Opts = maps:get(hackney_opts, Optional, []),
