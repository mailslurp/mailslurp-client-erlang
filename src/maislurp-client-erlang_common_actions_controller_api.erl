...
create_new_email_address(Ctx, Optional) ->
    ...
    Method = post,
    Path = ["/createInbox"],
    QS = lists:flatten([])++maislurp-client-erlang_utils:optional_params(['allowTeamAccess', 'useDomainPool', 'expiresAt', 'expiresIn', 'emailAddress', 'inboxType', 'description', 'name', 'tags', 'favourite'], _OptionalParams),
    Headers = [],
    Body1 = [],
...
%% @doc Create new random inbox
%% Returns an Inbox with an `id` and an `emailAddress`
-spec create_new_email_address1(ctx:ctx()) -> {ok, maislurp-client-erlang_inbox_dto:maislurp-client-erlang_inbox_dto(), maislurp-client-erlang_utils:response_info()} | {ok, hackney:client_ref()} | {error, term(), maislurp-client-erlang_utils:response_info()}.
create_new_email_address1(Ctx) ->
    create_new_email_address1(Ctx, #{}).

-spec create_new_email_address1(ctx:ctx(), maps:map()) -> {ok, maislurp-client-erlang_inbox_dto:maislurp-client-erlang_inbox_dto(), maislurp-client-erlang_utils:response_info()} | {ok, hackney:client_ref()} | {error, term(), maislurp-client-erlang_utils:response_info()}.
