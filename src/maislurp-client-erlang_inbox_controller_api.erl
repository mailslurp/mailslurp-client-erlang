...
create_inbox(Ctx, Optional) ->
    ...
    Method = post,
    Path = ["/inboxes"],
    QS = lists:flatten([])++maislurp-client-erlang_utils:optional_params(['emailAddress', 'tags', 'name', 'description', 'useDomainPool', 'favourite', 'expiresAt', 'expiresIn', 'allowTeamAccess', 'inboxType', 'virtualInbox', 'useShortAddress'], _OptionalParams),
    Headers = [],
    Body1 = [],
    ContentTypeHeader = maislurp-client-erlang_utils:select_header_content_type([]),
    Opts = maps:get(hackney_opts, Optional, []),
