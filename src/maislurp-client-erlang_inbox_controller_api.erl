...
create_inbox(Ctx, Optional) ->
    ...
    Method = post,
    Path = ["/inboxes"],
    QS = lists:flatten([])++maislurp-client-erlang_utils:optional_params(['emailAddress', 'tags', 'name', 'description', 'useDomainPool', 'favourite', 'expiresAt', 'expiresIn', 'allowTeamAccess', 'inboxType'], _OptionalParams),
    Headers = [],
    Body1 = [],
...
get_all_inboxes(Ctx, Optional) ->
    ...
    QS = lists:flatten([])++maislurp-client-erlang_utils:optional_params(['page', 'size', 'sort', 'favourite', 'search', 'tag', 'teamAccess', 'since', 'before'], _OptionalParams),
    Headers = [],
    Body1 = [],
    ContentTypeHeader = maislurp-client-erlang_utils:select_header_content_type([]),
    Opts = maps:get(hackney_opts, Optional, []),
...
get_emails(Ctx, InboxId, Optional) ->
    ...
    QS = lists:flatten([])++maislurp-client-erlang_utils:optional_params(['Alias for limit. Assessed first before assessing any passed limit.', 'limit', 'Sort the results by received date and direction ASC or DESC', 'retryTimeout', 'delayTimeout', 'minCount', 'unreadOnly', 'before', 'since'], _OptionalParams),
    Headers = [],
    Body1 = [],
    ContentTypeHeader = maislurp-client-erlang_utils:select_header_content_type([]),
    Opts = maps:get(hackney_opts, Optional, []),
