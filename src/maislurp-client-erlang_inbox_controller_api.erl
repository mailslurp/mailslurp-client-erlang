...
%% @doc Create an inbox email address. An inbox has a real email address and can send and receive emails.
...
create_inbox(Ctx, Optional) ->
    ...
    Method = post,
    Path = ["/inboxes"],
    QS = lists:flatten([])++maislurp-client-erlang_utils:optional_params(['allowTeamAccess', 'description', 'emailAddress', 'expiresAt', 'expiresIn', 'favourite', 'name', 'tags', 'useDomainPool'], _OptionalParams),
    Headers = [],
    Body1 = [],
