...
wait_for_email_count(Ctx, Optional) ->
    ...
    QS = lists:flatten([])++maislurp-client-erlang_utils:optional_params(['before', 'count', 'delay', 'inboxId', 'since', 'sort', 'timeout', 'unreadOnly'], _OptionalParams),
    Headers = [],
    Body1 = [],
...
wait_for_latest_email(Ctx, Optional) ->
    ...
    QS = lists:flatten([])++maislurp-client-erlang_utils:optional_params(['before', 'delay', 'inboxId', 'since', 'sort', 'timeout', 'unreadOnly'], _OptionalParams),
    Headers = [],
    Body1 = [],
    ContentTypeHeader = maislurp-client-erlang_utils:select_header_content_type([]),
    Opts = maps:get(hackney_opts, Optional, []),
...
wait_for_nth_email(Ctx, Optional) ->
    ...
    QS = lists:flatten([])++maislurp-client-erlang_utils:optional_params(['before', 'delay', 'inboxId', 'index', 'since', 'sort', 'timeout', 'unreadOnly'], _OptionalParams),
    Headers = [],
    Body1 = [],
    ContentTypeHeader = maislurp-client-erlang_utils:select_header_content_type([]),
    Opts = maps:get(hackney_opts, Optional, []),
