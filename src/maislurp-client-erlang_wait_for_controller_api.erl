...
%% @doc Wait for and return count number of emails 
...
wait_for_email_count(Ctx, Optional) ->
    ...
    QS = lists:flatten([])++maislurp-client-erlang_utils:optional_params(['count', 'inboxId', 'timeout', 'unreadOnly'], _OptionalParams),
    Headers = [],
    Body1 = [],
...
wait_for_latest_email(Ctx, Optional) ->
    ...
    QS = lists:flatten([])++maislurp-client-erlang_utils:optional_params(['inboxId', 'timeout', 'unreadOnly'], _OptionalParams),
    Headers = [],
    Body1 = [],
    ContentTypeHeader = maislurp-client-erlang_utils:select_header_content_type([]),
    Opts = maps:get(hackney_opts, Optional, []),
...
wait_for_matching_email(Ctx, MatchOptions, Optional) ->
    _OptionalParams = maps:get(params, Optional, #{}),
    Cfg = maps:get(cfg, Optional, application:get_env(kuberl, config, #{})),
...
%% @doc Wait for or return the first email that matches proved MatchOptions array
...
%% @doc Wait for or fetch the email with a given index in the inbox specified
...
wait_for_nth_email(Ctx, Optional) ->
    ...
    QS = lists:flatten([])++maislurp-client-erlang_utils:optional_params(['inboxId', 'index', 'timeout', 'unreadOnly'], _OptionalParams),
    Headers = [],
    Body1 = [],
    ContentTypeHeader = maislurp-client-erlang_utils:select_header_content_type([]),
    Opts = maps:get(hackney_opts, Optional, []),
