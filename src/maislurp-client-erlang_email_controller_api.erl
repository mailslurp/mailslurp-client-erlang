...
%% @doc Get email attachment as base64 encoded string alternative to binary responses. Decode the `base64FileContents` as a `utf-8` encoded string or array of bytes depending on the `contentType`.
...
%% @doc Get all email attachment metadata. Metadata includes name and attachment size.
...
%% @doc Get all emails in all inboxes. Email API list all.
...
get_emails_paginated(Ctx, Optional) ->
    ...
    Method = get,
    Path = ["/emails"],
    QS = lists:flatten([])++maislurp-client-erlang_utils:optional_params(['inboxId', 'page', 'size', 'sort', 'unreadOnly'], _OptionalParams),
    Headers = [],
    Body1 = [],
    ContentTypeHeader = maislurp-client-erlang_utils:select_header_content_type([]),
    Opts = maps:get(hackney_opts, Optional, []),
