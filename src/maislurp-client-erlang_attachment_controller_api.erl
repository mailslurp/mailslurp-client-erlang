...
-export([delete_attachment/2, delete_attachment/3,
...
get_attachments(Ctx, Optional) ->
    ...
    Method = get,
    Path = ["/attachments"],
    QS = lists:flatten([])++maislurp-client-erlang_utils:optional_params(['fileNameFilter', 'page', 'size', 'sort'], _OptionalParams),
    Headers = [],
    Body1 = [],
    ContentTypeHeader = maislurp-client-erlang_utils:select_header_content_type([]),
    Opts = maps:get(hackney_opts, Optional, []),
