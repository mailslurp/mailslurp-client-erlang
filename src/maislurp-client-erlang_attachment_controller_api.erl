...
%% 
-spec upload_attachment_bytes(ctx:ctx(), maislurp-client-erlang_inline_object_1:maislurp-client-erlang_inline_object_1()) -> {ok, [binary()], maislurp-client-erlang_utils:response_info()} | {ok, hackney:client_ref()} | {error, term(), maislurp-client-erlang_utils:response_info()}.
upload_attachment_bytes(Ctx, MaislurpClientErlangInlineObject1) ->
    upload_attachment_bytes(Ctx, MaislurpClientErlangInlineObject1, #{}).

-spec upload_attachment_bytes(ctx:ctx(), maislurp-client-erlang_inline_object_1:maislurp-client-erlang_inline_object_1(), maps:map()) -> {ok, [binary()], maislurp-client-erlang_utils:response_info()} | {ok, hackney:client_ref()} | {error, term(), maislurp-client-erlang_utils:response_info()}.
upload_attachment_bytes(Ctx, MaislurpClientErlangInlineObject1, Optional) ->
    _OptionalParams = maps:get(params, Optional, #{}),
    Cfg = maps:get(cfg, Optional, application:get_env(kuberl, config, #{})),

    Method = post,
    Path = ["/attachments/bytes"],
    QS = lists:flatten([])++maislurp-client-erlang_utils:optional_params(['filename'], _OptionalParams),
    Headers = []++maislurp-client-erlang_utils:optional_params(['contentType'], _OptionalParams),
