...
-export([delete_all_attachments/1, delete_all_attachments/2,
         ...
         extract_attachment_text/3, extract_attachment_text/4,
...
%% @doc Extract text from an attachment
...
-spec extract_attachment_text(ctx:ctx(), binary()) -> {ok, maislurp-client-erlang_extract_attachment_text_result:maislurp-client-erlang_extract_attachment_text_result(), maislurp-client-erlang_utils:response_info()} | {ok, hackney:client_ref()} | {error, term(), maislurp-client-erlang_utils:response_info()}.
extract_attachment_text(Ctx, AttachmentId) ->
    extract_attachment_text(Ctx, AttachmentId, #{}).

-spec extract_attachment_text(ctx:ctx(), binary(), maps:map()) -> {ok, maislurp-client-erlang_extract_attachment_text_result:maislurp-client-erlang_extract_attachment_text_result(), maislurp-client-erlang_utils:response_info()} | {ok, hackney:client_ref()} | {error, term(), maislurp-client-erlang_utils:response_info()}.
extract_attachment_text(Ctx, AttachmentId, Optional) ->
    _OptionalParams = maps:get(params, Optional, #{}),
    Cfg = maps:get(cfg, Optional, application:get_env(kuberl, config, #{})),

    Method = post,
    Path = ["/attachments/", AttachmentId, "/text"],
    QS = [],
    Headers = [],
    Body1 = MaislurpClientErlangExtractAttachmentTextOptions,
...
upload_multipart_form(Ctx, Optional) ->
    ...
    Headers = [],
    Body1 = MaislurpClientErlangInlineObject2,
    ContentTypeHeader = maislurp-client-erlang_utils:select_header_content_type([<<"application/json">>]),
    Opts = maps:get(hackney_opts, Optional, []),
