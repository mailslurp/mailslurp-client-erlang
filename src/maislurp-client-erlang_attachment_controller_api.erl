...
get_attachments(Ctx, Optional) ->
    ...
    Method = get,
    Path = ["/attachments"],
    QS = lists:flatten([])++maislurp-client-erlang_utils:optional_params(['page', 'size', 'sort', 'fileNameFilter', 'since', 'before', 'inboxId'], _OptionalParams),
    Headers = [],
    Body1 = [],
    ContentTypeHeader = maislurp-client-erlang_utils:select_header_content_type([]),
    Opts = maps:get(hackney_opts, Optional, []),
...
upload_attachment_bytes(Ctx, Optional) ->
    ...
    QS = lists:flatten([])++maislurp-client-erlang_utils:optional_params(['contentType', 'contentId', 'filename'], _OptionalParams),
...
upload_multipart_form(Ctx, Optional) ->
    ...
    QS = lists:flatten([])++maislurp-client-erlang_utils:optional_params(['contentId', 'contentType', 'filename', 'x-filename'], _OptionalParams),
    Headers = [],
