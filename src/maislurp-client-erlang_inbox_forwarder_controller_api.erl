...
create_new_inbox_forwarder(Ctx, MaislurpClientErlangCreateInboxForwarderOptions) ->
    create_new_inbox_forwarder(Ctx, MaislurpClientErlangCreateInboxForwarderOptions, #{}).
...
create_new_inbox_forwarder(Ctx, MaislurpClientErlangCreateInboxForwarderOptions, Optional) ->
...
get_all_inbox_forwarder_events(Ctx, Optional) ->
    ...
    QS = lists:flatten([])++maislurp-client-erlang_utils:optional_params(['page', 'size', 'inboxId', 'emailId', 'sentId', 'sort'], _OptionalParams),
    Headers = [],
    Body1 = [],
    ContentTypeHeader = maislurp-client-erlang_utils:select_header_content_type([]),
    Opts = maps:get(hackney_opts, Optional, []),
