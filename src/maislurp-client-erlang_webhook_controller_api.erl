...
-export([create_webhook/3, create_webhook/4,
         ...
         get_webhook_results_unseen_error_count/2, get_webhook_results_unseen_error_count/3,
...
get_all_webhook_results(Ctx, Optional) ->
    ...
    QS = lists:flatten([])++maislurp-client-erlang_utils:optional_params(['before', 'page', 'searchFilter', 'since', 'size', 'sort', 'unseenOnly'], _OptionalParams),
    Headers = [],
    Body1 = [],
    ContentTypeHeader = maislurp-client-erlang_utils:select_header_content_type([]),
    Opts = maps:get(hackney_opts, Optional, []),
...
%% @doc Get JSON Schema definition for webhook payload
%% 
...
%% @doc Get example payload for webhook
%% 
...
%% 
-spec get_webhook_results_unseen_error_count(ctx:ctx(), maislurp-client-erlang_u_uid:maislurp-client-erlang_u_uid()) -> {ok, maislurp-client-erlang_unseen_error_count_dto:maislurp-client-erlang_unseen_error_count_dto(), maislurp-client-erlang_utils:response_info()} | {ok, hackney:client_ref()} | {error, term(), maislurp-client-erlang_utils:response_info()}.
get_webhook_results_unseen_error_count(Ctx, InboxId) ->
    get_webhook_results_unseen_error_count(Ctx, InboxId, #{}).

-spec get_webhook_results_unseen_error_count(ctx:ctx(), maislurp-client-erlang_u_uid:maislurp-client-erlang_u_uid(), maps:map()) -> {ok, maislurp-client-erlang_unseen_error_count_dto:maislurp-client-erlang_unseen_error_count_dto(), maislurp-client-erlang_utils:response_info()} | {ok, hackney:client_ref()} | {error, term(), maislurp-client-erlang_utils:response_info()}.
get_webhook_results_unseen_error_count(Ctx, InboxId, Optional) ->
    _OptionalParams = maps:get(params, Optional, #{}),
    Cfg = maps:get(cfg, Optional, application:get_env(kuberl, config, #{})),
