...
describe_mail_server_domain(Ctx, DescribeOptions) ->
    describe_mail_server_domain(Ctx, DescribeOptions, #{}).
...
describe_mail_server_domain(Ctx, DescribeOptions, Optional) ->
...
get_dns_lookup(Ctx, DnsLookupOptions) ->
    get_dns_lookup(Ctx, DnsLookupOptions, #{}).
...
get_dns_lookup(Ctx, DnsLookupOptions, Optional) ->
    _OptionalParams = maps:get(params, Optional, #{}),
    Cfg = maps:get(cfg, Optional, application:get_env(kuberl, config, #{})),
...
%% @doc Verify the existence of an email address at a given mail server.
%% 
...
verify_email_address(Ctx, VerifyOptions) ->
    verify_email_address(Ctx, VerifyOptions, #{}).
...
verify_email_address(Ctx, VerifyOptions, Optional) ->
    _OptionalParams = maps:get(params, Optional, #{}),
    Cfg = maps:get(cfg, Optional, application:get_env(kuberl, config, #{})),
