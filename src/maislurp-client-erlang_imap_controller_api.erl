...
-export([imap_server_fetch/2, imap_server_fetch/3,
         imap_server_get/2, imap_server_get/3,
         imap_server_list/2, imap_server_list/3,
         imap_server_mailbox/2, imap_server_mailbox/3,
...
%% @doc Get a message by email ID
%% 
-spec imap_server_get(ctx:ctx(), maislurp-client-erlang_u_uid:maislurp-client-erlang_u_uid()) -> {ok, maislurp-client-erlang_imap_server_get_result:maislurp-client-erlang_imap_server_get_result(), maislurp-client-erlang_utils:response_info()} | {ok, hackney:client_ref()} | {error, term(), maislurp-client-erlang_utils:response_info()}.
imap_server_get(Ctx, EmailId) ->
    imap_server_get(Ctx, EmailId, #{}).

-spec imap_server_get(ctx:ctx(), maislurp-client-erlang_u_uid:maislurp-client-erlang_u_uid(), maps:map()) -> {ok, maislurp-client-erlang_imap_server_get_result:maislurp-client-erlang_imap_server_get_result(), maislurp-client-erlang_utils:response_info()} | {ok, hackney:client_ref()} | {error, term(), maislurp-client-erlang_utils:response_info()}.
imap_server_get(Ctx, EmailId, Optional) ->
    _OptionalParams = maps:get(params, Optional, #{}),
    Cfg = maps:get(cfg, Optional, application:get_env(kuberl, config, #{})),

    Method = post,
    Path = ["/imap/server/get"],
    QS = lists:flatten([{<<"emailId">>, EmailId}])++maislurp-client-erlang_utils:optional_params(['inboxId'], _OptionalParams),
    Headers = [],
    Body1 = [],
    ContentTypeHeader = maislurp-client-erlang_utils:select_header_content_type([]),
    Opts = maps:get(hackney_opts, Optional, []),
...
%% @doc Create a new mailbox if possible
%% 
-spec imap_server_mailbox(ctx:ctx(), binary()) -> {ok, maislurp-client-erlang_imap_server_mailbox_result:maislurp-client-erlang_imap_server_mailbox_result(), maislurp-client-erlang_utils:response_info()} | {ok, hackney:client_ref()} | {error, term(), maislurp-client-erlang_utils:response_info()}.
imap_server_mailbox(Ctx, Name) ->
    imap_server_mailbox(Ctx, Name, #{}).

-spec imap_server_mailbox(ctx:ctx(), binary(), maps:map()) -> {ok, maislurp-client-erlang_imap_server_mailbox_result:maislurp-client-erlang_imap_server_mailbox_result(), maislurp-client-erlang_utils:response_info()} | {ok, hackney:client_ref()} | {error, term(), maislurp-client-erlang_utils:response_info()}.
imap_server_mailbox(Ctx, Name, Optional) ->
    _OptionalParams = maps:get(params, Optional, #{}),
    Cfg = maps:get(cfg, Optional, application:get_env(kuberl, config, #{})),
